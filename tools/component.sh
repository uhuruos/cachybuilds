...
_help() {
    ...
    echo
    echo "Scripts that perform operations related to components" 
    echo
    echo " General command:"
    echo "    check [name]              Determine if the locale is available"
    echo "    show                      Shows a list of available components"
...
check(){
    ...
    local _version
    if [[ -f "${component_dir}/${1}/alteriso" ]]; then
        _version="$(
            source "${component_dir}/${1}/alteriso"
            echo "${alteriso}"
            unset alteriso
        )"
...
show(){
    local _component _name _version _list=()
    while read -r _component; do
        _name="$(basename "$(dirname "${_component}")")"
        _version="$(
            source "${_component}"
            echo "${alteriso}"
            unset alteriso
    ...
    done < <(find "${component_dir}" -maxdepth 2 -mindepth 2 -type f -name "alteriso")
    echo "${_list[@]}"
}
