...
_help() {
    ...
    echo "    -m | --component       [str]      Specity the component (Separated by \",\")"
...
OPTL="arch:,boot-splash:,channel:,developer:,kernel:,component:,os-name:,password:,username:,version:,help"
...
while true; do
    case "${1}" in
        ...
        -k | --kernel)
            ...
            ;;
        -m | --component)
            readarray -t -O "${#components[@]}" components < <(echo "${2}" | tr "," "\n")
            shift 2
            ;;
        -o | --os-name)
            iso_application="${2}"
...
[[ "${#components[@]}" != 0 ]] && echo "Loaded components : ${components[*]}"
