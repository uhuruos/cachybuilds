...
_help() {
    ...
    echo "    -v | --version [ver]      Specifies the archiso version"
...
gen_channel_list() {
    ...
    for _dirname in $(ls -l "${script_path}"/channels/ | awk '$1 ~ /d/ {print $9}'); do
        if [[ -n $(ls "${script_path}"/channels/${_dirname}) ]] && check_archiso_version "${_dirname}/" || [[ "${opt_nochkver}" = true ]]; then
...
# check?archiso_version <channel dir>
get_archiso_version(){
    local _channel
...
check_archiso_version(){
    #if [[ "$(get_archiso_version "${1%.add}")" = "${archiso_version}" ]]; then
    if [[ "$(get_archiso_version "${1%.add}" | cut -d "." -f 1)" = "$(echo "${archiso_version}" | cut -d "." -f 1)" ]]; then
        return 0
    else
        return 1
    fi
...
check() {
    ...
    if [[ $(printf '%s\n' "${channellist[@]}" | grep -qx "${1}"; echo -n ${?} ) -eq 0 ]]; then
        ...
        if check_archiso_version "${_channel_name}" || [[ "${opt_nochkver}" = true ]]; then
            #echo "directory"
            exit 1
        else
            #echo "incorrect"
...
desc() {
    ...
    if ! check_archiso_version "${_channel}" && [[ "${opt_nochkver}" = false ]]; then
        "${script_path}/tools/msg.sh" --noadjust -l 'ERROR:' --noappname error "Not compatible with archiso3"
